...
export const activityReducer = (state={}, action)=>{
    switch(action.type){
        ...
        case CREATE_USER_ACTIVITY_SUCCESS:
            return {...state, loading:false, userActivity:{...state.userActivity,score: action.payload.finished? state.userActivity.score+action.payload.score : state.userActivity.score,activities:[action.payload,...state.userActivity.activities ]}}
        ...
        case EDIT_USER_ACTIVITY_SUCCESS:
            return {...state, loading:false, userActivity:{...state.userActivity, activities:state.userActivity.activities.map(activity=>activity.id===action.payload.id? action.payload : activity)}}
        ...
        case DELETE_USER_ACTIVITY_SUCCESS:
            return {...state, loading:false, userActivity:{...state.userActivity, score:action.payload.finished?state.userActivity.score-action.payload.score : state.userActivity.score,activities:state.userActivity.activities.filter(activity=> activity.id!==action.payload.id)}}
